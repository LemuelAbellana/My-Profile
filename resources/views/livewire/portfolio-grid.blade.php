<div>
    ...
    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        @foreach($portfolios as $portfolio)
            <div 
                wire:click="viewProject({{ $portfolio->id }})"
                ...
                <div class="p-6">
                    <div class="flex items-start justify-between mb-2">
                        <h3 class="font-bold text-xl">{{ $portfolio->title }}</h3>
                    ...
                    <div class="space-y-2 mb-4">
                        ...
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <span class="font-semibold {{ $portfolio->tests_passing ? 'text-green-600' : 'text-red-600' }}">
                                {{ $portfolio->tests_passing ? '🟢 Passing' : '🔴 Failing' }}
                            </span>
                        </div>
                        ...
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <span class="font-semibold">
                                🛡️ {{ $portfolio->critical_bugs }} Critical
                                @if($portfolio->minor_bugs > 0)
                                    , {{ $portfolio->minor_bugs }} Minor
                                @endif
                            </span>
                        </div>
                        ...
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <div class="flex items-center gap-2">
                                <div class="w-24 bg-gray-200 rounded-full h-2">
                                    <div 
                                        class="h-2 rounded-full {{ $portfolio->performance_score >= 90 ? 'bg-green-500' : ($portfolio->performance_score >= 70 ? 'bg-yellow-500' : 'bg-red-500') }}" 
                                        style="width: {{ $portfolio->performance_score }}%">
                                    </div>
                                </div>
                                <span class="font-semibold">{{ $portfolio->performance_score }}/100</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-4">
                        <span class="status-badge {{ $portfolio->health_color }}">
                            {{ strtoupper($portfolio->health_status) }}
                        </span>
                    </div>
                </div>
            </div>
    ...
    @if($showModal && $selectedProject)
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 overflow-y-auto" wire:click="closeModal">
            <div class="bg-white rounded-lg max-w-4xl w-full p-8 my-8" wire:click.stop>
                <div class="flex justify-between items-start mb-6">
                    <div>
                        ...
                        <div class="flex gap-2">
                            <span class="status-badge {{ $selectedProject->health_color }}">
                                {{ strtoupper($selectedProject->health_status) }}
                            </span>
                            @if($selectedProject->is_featured)
                    ...
                    <div class="grid md:grid-cols-3 gap-4">
                        <div class="text-center">
                            <div class="text-3xl mb-2">
                                {{ $selectedProject->tests_passing ? '🟢' : '🔴' }}
                            </div>
                            <div class="font-semibold">{{ $selectedProject->build_status }}</div>
                            <div class="text-sm text-gray-600">Build Status</div>
                        </div>
                        <div class="text-center">
                            <div class="text-3xl mb-2">🛡️</div>
                            <div class="font-semibold">{{ $selectedProject->critical_bugs }} Critical</div>
                            <div class="text-sm text-gray-600">{{ $selectedProject->minor_bugs }} Minor Bugs</div>
                        </div>
                        <div class="text-center">
                            <div class="text-3xl mb-2">⚡</div>
                            <div class="font-semibold">{{ $selectedProject->performance_score }}/100</div>
                ...
                <!-- Case Study -->
                @if($selectedProject->challenge || $selectedProject->solution || $selectedProject->results)
                    <div class="space-y-4 mb-6">
                        @if($selectedProject->challenge)
                            <div>
                                <h4 class="font-bold mb-2">The Challenge</h4>
                                <p class="text-gray-700">{{ $selectedProject->challenge }}</p>
                            </div>
                        @endif
                        ...
                        @if($selectedProject->solution)
                            <div>
                                ...
                                <p class="text-gray-700">{{ $selectedProject->solution }}</p>
                            </div>
                        @endif
                        ...
                        @if($selectedProject->results)
                            <div>
                                ...
                                <p class="text-gray-700">{{ $selectedProject->results }}</p>
                            </div>
                        @endif
                    </div>
                @endif
