    ...
    @if($submitted)
        ...
        <div class="bg-green-50 border-2 border-green-500 rounded-lg p-8 text-center">
            ...
            <div class="bg-white rounded-lg p-4 inline-block">
                ...
                <p class="text-2xl font-mono font-bold text-green-600">{{ $ticketNumber }}</p>
            </div>
        </div>
    @else
        <!-- Contact Form -->
            ...
            <form wire:submit.prevent="submitTicket">
                ...
                <div class="mb-6">
                    <label class="block text-sm font-semibold mb-2">
                        <span class="client-only">Name *</span>
                ...
                <div class="flex gap-4">
                    ...
                    <button 
                        type="submit"
                        class="flex-1 px-8 py-3 font-semibold rounded-lg transition {{ $severity === 'critical' ? 'bg-red-500 hover:bg-red-600 text-white animate-pulse' : 'bg-client-500 hover:bg-client-600 text-white' }}">
                        <span class="client-only">
                            {{ $severity === 'critical' ? '🚀 Deploy to Inbox (URGENT!)' : 'Send Message' }}
                        </span>
                        <span class="dev-only hidden">
                            {{ $severity === 'critical' ? 'deploy("CRITICAL")' : 'submit()' }}
                        </span>
                    </button>
                </div>
